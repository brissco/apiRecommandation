...
#         print(f"les profil des utilisateur sont {worker_profiles}")
#         mission_data = request.data.get('mission_data', [])
#         print(f"les anciennes missions sont {mission_data}")
#         new_missions = request.data.get('new_missions', [])
#         print(f"les nouvelle mission sont {worker_profiles}")
#         worker_histories = request.data.get('worker_histories', {})
#         print(f"l'historique des missions sont {worker_profiles}")
...
class RecommendationView(APIView):
    def post(self, request):
        ...
        # Vérification et correction des types de `history`
        for worker_id, profile in worker_profiles.items():
            if not isinstance(profile.get('history', {}), dict):
                profile['history'] = {}
    ...
    def hybrid_recommendation(self, worker_id, worker_profiles, missions, worker_histories, new_mission_ids):
        worker_vector = np.random.rand(1, 10)  # Exemple de vecteur travailleur
        mission_vectors = np.random.rand(len(missions), 10)  # Exemple de vecteurs de missions
        mission_ids = [mission['id'] for mission in missions]
    ...
    def aggregate_recommendations(mission_histories, similar_workers):
        recommendations = defaultdict(float)
        for worker in similar_workers:
            if worker in mission_histories:
                for mission, rating in mission_histories[worker].items():
                    recommendations[mission] += rating
        return sorted(recommendations.items(), key=lambda x: x[1], reverse=True)
    ...
    def recommend_workers_for_mission(self, mission_id, worker_profiles, all_missions, worker_histories):
        mission = next((mission for mission in all_missions if mission['id'] == mission_id), None)
        if not mission:
            return []

        mission_vector = np.random.rand(1, 10)  # Exemple de vecteur de mission
        worker_vectors = np.random.rand(len(worker_profiles), 10)  # Exemple de vecteurs de travailleurs
        worker_ids = list(worker_profiles.keys())

        content_based = self.content_based_recommendation(mission_vector, worker_vectors, worker_ids)
        combined_scores = defaultdict(float)
        for worker_id, score in content_based:
            combined_scores[worker_id] += score
